...
// Safety: all mutable state is accessed from the internal serial queue, and
// start/stop are invoked from the app coordinator on the main actor.
final class IPCServer: @unchecked Sendable {
    private let socketPath: String
    private let handler: (IPCRequest) -> IPCResponse
    private let queue = DispatchQueue(label: "idx0.ipc.server", qos: .userInitiated)
    ...
    private func handleClient(fd: Int32) {
        ...
        do {
            ...
            let request = try decoder.decode(IPCRequest.self, from: requestBuffer)
