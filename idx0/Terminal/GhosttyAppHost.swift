...
final class GhosttyAppHost {
    ...
    func initialize() {
        ...
        guard idx0_ghostty_load(&maybeError) else {
        ...
        let loadPath = idx0_ghostty_loaded_path().map { String(cString: $0) } ?? "dynamic"
        let initResult = idx0_ghostty_init(UInt(CommandLine.argc), CommandLine.unsafeArgv)
        ...
        guard let config = idx0_ghostty_config_new() else {
            availability = .unavailable("ghostty_config_new failed")
            return
        }

        idx0_ghostty_config_load_default_files(config)
        ...
        idx0_ghostty_config_finalize(config)
        ...
        runtimeConfig.read_clipboard_cb = { userdata, _, state in
            ...
            value.withCString { cString in
                idx0_ghostty_surface_complete_clipboard_request(surface, cString, state, false)
            }
        }
        runtimeConfig.confirm_read_clipboard_cb = { userdata, content, state, _ in
            ...
            let context = Unmanaged<GhosttySurfaceCallbackContext>.fromOpaque(userdata).takeUnretainedValue()
            guard let surface = context.surface?.surface else { return }
            idx0_ghostty_surface_complete_clipboard_request(surface, content, state, true)
        }
        ...
        guard let app = idx0_ghostty_app_new(&runtimeConfig, config) else {
            availability = .unavailable("ghostty_app_new failed")
            idx0_ghostty_config_free(config)
            return
        }
    ...
    func createSurface(for wrapper: GhosttyTerminalSurface) {
        ...
        var config = idx0_ghostty_surface_config_new()
        ...
        let surface: ghostty_surface_t? = wrapper.workingDirectory.withCString { cwdCString in
            if let command {
                return command.withCString { shellCString in
                    ...
                    return idx0_ghostty_surface_new(app, &config)
                }
            } else {
                config.working_directory = cwdCString
                config.command = nil
        ...
        if let screen = view.window?.screen ?? NSScreen.main,
           let displayID = screen.deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? NSNumber {
            idx0_ghostty_surface_set_display_id(surface, displayID.uint32Value)
        }
        ...
        idx0_ghostty_surface_set_content_scale(surface, xScale, yScale)
        let wpx = UInt32(max(1, Int(floor(backingSize.width))))
        let hpx = UInt32(max(1, Int(floor(backingSize.height))))
        idx0_ghostty_surface_set_size(surface, wpx, hpx)
    ...
    func blurSurface(_ wrapper: GhosttyTerminalSurface) {
        guard let surface = wrapper.surface else { return }
        idx0_ghostty_surface_set_focus(surface, false)
        scheduleTick()
    }
    ...
    func dumpScrollback(_ wrapper: GhosttyTerminalSurface) -> String? {
        ...
        guard idx0_ghostty_surface_read_text(surface, sel, &textResult) else { return nil }
        defer { idx0_ghostty_surface_free_text(surface, &textResult) }
    ...
    func sendText(_ text: String, to wrapper: GhosttyTerminalSurface) {
        ...
        text.utf8CString.withUnsafeBufferPointer { buffer in
            ...
            idx0_ghostty_surface_text(surface, base, UInt(buffer.count - 1))
        }
        scheduleTick()
    }
    ...
    private func setAppFocus(_ focused: Bool) {
        guard let app else { return }
        idx0_ghostty_app_set_focus(app, focused)
        scheduleTick()
    }
    ...
    private static var themeConfigPath: String {
        ...
        let dir = support.appendingPathComponent("idx0", isDirectory: true)
    ...
    private func loadThemeConfig(into config: ghostty_config_t) {
        ...
        path.withCString { cPath in
            idx0_ghostty_config_load_file(config, cPath)
        }
    ...
    static func writeThemeConfig(themeID: String?) {
        ...
        guard let themeID, let theme = TerminalTheme.theme(withID: themeID) else {
            // No theme selected — remove the file so Ghostty uses defaults
            try? FileManager.default.removeItem(atPath: path)
            return
        }
        let content = theme.ghosttyConfig()
